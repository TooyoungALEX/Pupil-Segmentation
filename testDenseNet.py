...
def main():
    ...
    model_file_name = "efe-DenseNet-trained-model-640x480.pt"  # 这是用于椭圆拟合误差的模型
    ...
    for dir in range(1, 23, 1):
        for user in users[dir]:
            ...
            while True:
                ...
                output_bk = predict.cpu().numpy()

                ## 连通组件分析
                if np.count_nonzero(output_bk) != 0:
                    no_labels, labels, stats, center = cv2.connectedComponentsWithStats(
                ...
                if frameNo % 500 == 0:
                    print(
                        f"frameNo: {frameNo}, no_images_with_more_than_one_blob: {no_images_with_more_than_one_blob}");
